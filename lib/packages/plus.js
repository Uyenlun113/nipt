...
export const genetPlusPackageHandler = {
  ...
  async parsePdf(pdfBuffer) {
    ...
    const extractTrisomy = (pattern) => {
      ...
      while ((m = regex.exec(text)) !== null) {
        ...
        // Search for signed decimal float (e.g. -2,015, –2,015, —2,015, -0,472, 1,152, 0.076)
        // Skipping integer bounds like -3 and 3 in reference range (-3 < Z < 3)
  ...
  async generatePdf(sampleData) {
    ...
    const renderRow1 = (itemKey, y) => {
      ...
      drawCenterTextOnPage(page1, item.value || '', TABLE_VAL_CENTER_X, y, fontBold, 9.5, isAlert ? alertColor : darkColor);
      drawCenterTextOnPage(page1, item.risk || 'Nguy cơ thấp', TABLE_RISK_CENTER_X, y, fontBold, 9.5, isAlert ? alertColor : textColor);
    };

    // Section I (3 Main Trisomies)
    ...
    const defaultPlusConclusion = 'Bộ nhiễm sắc thể người bình thường bao gồm 23 cặp, trong đó có 22 cặp Nhiễm sắc thể thường và 1 cặp nhiễm sắc thể giới tính. Mỗi cặp có 2 nhiễm sắc thể. Kết quả NIPT nguy cơ thấp phản ánh không có bất thường về số lượng Nhiễm sắc thể đối với các cặp Nhiễm sắc thể được kiểm tra. Mất đoạn, lặp đoạn xảy ra do mất hoặc thêm vật chất di truyền ở trên một nhiễm sắc thể. Các bất thường này có thể gây ra các khuyết tật bẩm sinh. Kết quả xét nghiệm “Không phát hiện” phản ánh không phát hiện các bất thường vi mất, lặp đoạn nằm trong khả năng bao phủ của xét nghiệm.';
    const conclusionStr = sampleData.conclusion !== undefined && sampleData.conclusion !== null ? sampleData.conclusion : defaultPlusConclusion;
    ...
    page1.drawText(conclusionStr, { x: 98, y: 316.4, size: 8, font: fontBold, color: isConcAbnormal ? alertColor : rgb(0.05, 0.25, 0.55), maxWidth: 450, lineHeight: 11 });
    ...
    if (page2) {
      ...
      for (const [key, yPos] of Object.entries(otherTrisomiesCoords)) {
        ...
        if (item) {
          const isAlert = (item.risk || '').toLowerCase().includes('cao');
          drawCenterTextOnPage(page2, item.value || '', TABLE_VAL_CENTER_X, yPos, fontBold, 9.5, isAlert ? alertColor : darkColor);
