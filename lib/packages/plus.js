  ...
  async generatePdf(sampleData) {
    ...
    const normalizeStr = (str) => {
      ...
      return String(str)
        ...
        .replace(/[ýỳỷỹỵ]/g, 'y')
        .replace(/đ/g, 'd')
        .replace(/hoi\s*chung|vi\s*mat\s*doan|vi\s*lap\s*doan|mat\s*doan|lap\s*doan|syndrome|deletion|duplication|chromosome|microdeletion|triplication/g, '')
        .replace(/[^a-z0-9]/g, '');
    };
    ...
    MICRO_LIST.forEach((microItem, listIdx) => {
      ...
      const foundMatch = microResults.find((m, mIdx) => {
        if (!m) return false;
        if (mIdx === listIdx) return true;
        const mName = typeof m === 'string' ? m : (m.name || '');
        if (!mName) return false;

        const mNorm = normalizeStr(mName);
        if (mNorm && itemNorm && (mNorm === itemNorm || mNorm.includes(itemNorm) || itemNorm.includes(mNorm))) {
          return true;
        }

        const itemBand = (microItem.name.match(/[0-9]+[pq][0-9.]*/i) || [])[0];
        const mBand = (mName.match(/[0-9]+[pq][0-9.]*/i) || [])[0];
        if (itemBand && mBand && itemBand.toLowerCase() === mBand.toLowerCase()) {
          return true;
        }
