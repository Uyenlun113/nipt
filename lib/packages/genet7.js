...
export const genet7PackageHandler = {
  ...
  async parsePdf(pdfBuffer) {
    ...
    const formatZScore = (rawNum) => {
      if (!rawNum) return '';
      let str = rawNum.replace(',', '.').trim();
      if (/^-?0\d{2,3}$/.test(str)) {
        const isNeg = str.startsWith('-');
        const digits = isNeg ? str.slice(1) : str;
        str = (isNeg ? '-' : '') + digits[0] + '.' + digits.slice(1);
      }
      return str;
    };

    const extractTrisomy = (pattern) => {
      const regex = new RegExp('(?:' + pattern + ')[\\s\\S]{0,350}', 'gi');
      const matches = [];
      let m;
      while ((m = regex.exec(text)) !== null) {
        let chunk = m[0];
        const nextHeaderIdx = chunk.slice(25).search(/(?:Trisomy|HC\s|Hội\s*chứng|II\.|III\.|IV\.)/i);
        if (nextHeaderIdx !== -1) {
          chunk = chunk.slice(0, 25 + nextHeaderIdx);
        }
        matches.push(chunk);
      }

      for (const rawChunk of matches) {
        let cleanedChunk = rawChunk.replace(/-?\s*3\s*<\s*Z(?:-score)?\s*<\s*3/gi, '')
          .replace(/Z(?:-score)?\s*<\s*3/gi, '')
          .replace(/Lệch\s*bội\s*nhiễm\s*sắc\s*thể/gi, '')
          .replace(/(?:Trisomy|HC|Hội\s*chứng)\s*(?:21|18|13|47|45)?/gi, '')
          .replace(/\b(21|18|13|47|45|XO|XXY|XYY|XXX)\b/gi, '');

        let risk = '';
        if (/nguy\s*cơ\s*cao/i.test(cleanedChunk)) {
          risk = 'Nguy cơ cao';
        } else if (/nguy\s*cơ\s*thấp/i.test(cleanedChunk)) {
          risk = 'Nguy cơ thấp';
        }

        let value = '';
        const numMatch = cleanedChunk.match(/(-?[0-9]+[.,]?[0-9]*)/g);
        if (numMatch) {
          for (const rawNum of numMatch) {
            if (!rawNum || rawNum === '-') continue;
            const cleanedVal = formatZScore(rawNum);
            if (cleanedVal.includes('.') || cleanedVal.length >= 2 || parseFloat(cleanedVal) !== parseInt(cleanedVal, 10)) {
              value = cleanedVal;
              break;
            }
          }
    ...
    const klin = extractTrisomy('Klinefelter|47,\\s*XXY|\\(XXY\\)|XXY');
    if (klin) {
      results.klinefelter.value = klin.value;
      results.klinefelter.risk = klin.risk;
    }

    const jacobs = extractTrisomy('Jacobs|47,\\s*XYY|\\(XYY\\)|XYY');
    if (jacobs) {
      results.jacobs.value = jacobs.value;
      results.jacobs.risk = jacobs.risk;
    }

    const xxx = extractTrisomy('siêu\\s*nữ|Trisomy\\s*X|47,\\s*XXX|\\(XXX\\)|XXX|Triple\\s*X');
    if (xxx) {
  ...
  async generatePdf(sampleData) {
    const pdfBuffer = getCachedPdfTemplate(this.templateFileName, 'KQ_NIPT_GENET 7.pdf');
    ...
    const renderRow = (itemKey, y) => {
      const item = res[itemKey];
      if (!item) return;
      const isAlert = (item.risk || '').toLowerCase().includes('cao');
      if (isAlert) hasHighRisk = true;
      drawCenterText(item.value || '', TABLE_VAL_CENTER_X, y, fontBold, 9.5, isAlert ? alertColor : darkColor);
      drawCenterText(item.risk || '', TABLE_RISK_CENTER_X, y, fontBold, 9.5, isAlert ? alertColor : textColor);
    };
    ...
    renderRow('turner', 380.3);
    renderRow('klinefelter', 364.2);
    renderRow('jacobs', 348.1);
    renderRow('tripleX', 332.3);
    ...
    firstPage.drawText(conclusionStr, { x: 98, y: 316.4, size: 9, font: fontBold, color: conclusionColor, maxWidth: 450, lineHeight: 13 });
