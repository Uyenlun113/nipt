...
export const thalassemiaPackageHandler = {
  ...
  async parsePdf(pdfBuffer) {
    ...
    let conclusion = '';
    const klMatch = text.match(/KẾT\s*LUẬN\s*:?[\s\S]*?(?=LƯU\s*Ý|Ngày|KIỂM\s*SOÁT|Hà\s*Nội|$)/i);
    if (klMatch) {
  ...
  async generatePdf(sampleData) {
    ...
    const isAlphaMutated = valAlpha && !valAlpha.toLowerCase().includes('chưa phát hiện đột biến') && !valAlpha.toLowerCase().includes('chưa phát hiện biến thể');
    const isBetaMutated = valBeta && !valBeta.toLowerCase().includes('chưa phát hiện đột biến') && !valBeta.toLowerCase().includes('chưa phát hiện biến thể');

    const TABLE_CENTER_X = 472.5;

    // Row 1 (Alpha-thalassemia)
    drawCenterWrappedText(valAlpha, TABLE_CENTER_X, 538.0, isAlphaMutated ? fontBold : font, 8.5, isAlphaMutated ? alertColor : darkColor, 170, 11);

    // Row 2 (Beta-thalassemia - vertically centered in 45pt cell)
    drawCenterWrappedText(valBeta, TABLE_CENTER_X, 497.0, isBetaMutated ? fontBold : font, 8.5, isBetaMutated ? alertColor : darkColor, 170, 11);

    // 4. KẾT LUẬN (Same line right after "KẾT LUẬN:" header at x = 98, y = 448.0)
    let conclusionStr = sampleData.conclusion !== undefined && sampleData.conclusion !== null ? sampleData.conclusion : 'Chưa phát hiện biến thể gây bệnh/ có thể gây bệnh trên các vùng gen được khảo sát.';
    conclusionStr = (conclusionStr || '').replace(/^(KẾT\s*LUẬN|Kết\s*quả)\s*:?\s*/i, '').trim();
    const concLower = conclusionStr.toLowerCase();
    const isConcMutated = isAlphaMutated || isBetaMutated || (!concLower.includes('chưa phát hiện') && !concLower.includes('chua phat hien'));

    page1.drawText(conclusionStr, {
      x: 98,
      y: 448.0,
      size: 9.5,
      font: isConcMutated ? fontBold : font,
      color: isConcMutated ? alertColor : darkColor,
      maxWidth: 460
    });
