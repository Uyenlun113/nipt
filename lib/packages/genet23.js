...
export const genet23PackageHandler = {
  ...
  async generatePdf(sampleData) {
    ...
    if (res.t21) {
      drawCenterText(firstPage, res.t21.value || '', TABLE_VAL_CENTER_X, 489.3, fontBold, 9.5, darkColor);
    ...
    if (res.t18) {
      drawCenterText(firstPage, res.t18.value || '', TABLE_VAL_CENTER_X, 470.8, fontBold, 9.5, darkColor);
    ...
    if (res.t13) {
      drawCenterText(firstPage, res.t13.value || '', TABLE_VAL_CENTER_X, 452.3, fontBold, 9.5, darkColor);
    ...
    if (res.turner) {
      drawCenterText(firstPage, res.turner.value || '', TABLE_VAL_CENTER_X, 380.3, fontBold, 9, darkColor);
      drawCenterText(firstPage, res.turner.risk || 'Nguy cơ thấp', TABLE_RISK_CENTER_X, 380.3, fontBold, 9, textColor);
    }
    if (res.klinefelter) {
      drawCenterText(firstPage, res.klinefelter.value || '', TABLE_VAL_CENTER_X, 364.2, fontBold, 9, darkColor);
      drawCenterText(firstPage, res.klinefelter.risk || 'Nguy cơ thấp', TABLE_RISK_CENTER_X, 364.2, fontBold, 9, textColor);
    }
    if (res.jacobs) {
      drawCenterText(firstPage, res.jacobs.value || '', TABLE_VAL_CENTER_X, 348.1, fontBold, 9, darkColor);
      drawCenterText(firstPage, res.jacobs.risk || 'Nguy cơ thấp', TABLE_RISK_CENTER_X, 348.1, fontBold, 9, textColor);
    }
    if (res.tripleX) {
      drawCenterText(firstPage, res.tripleX.value || '', TABLE_VAL_CENTER_X, 332.3, fontBold, 9, darkColor);
    ...
    if (secondPage) {
      ...
      Object.entries(page2YMap).forEach(([key, yPos]) => {
        const item = res[key];
        if (item) {
          drawCenterText(secondPage, item.value || '', TABLE_VAL_CENTER_X, yPos, font, 9, darkColor);
