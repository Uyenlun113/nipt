...
export const genet23PackageHandler = {
  ...
  // 1. Default 23 Chromosome Disorder Rows for GeneT 23
  getDefaultResults() {
    const res = {
      t21: { label: 'Trisomy 21 (Down)', value: '-0.243', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      t18: { label: 'Trisomy 18 (Edwards)', value: '0.941', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      t13: { label: 'Trisomy 13 (Patau)', value: '0.020', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      turner: { label: 'HC Turner (45, XO)', value: '2.364', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      klinefelter: { label: 'HC Klinefelter (47, XXY)', value: '2.364', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      jacobs: { label: 'HC Jacobs (47, XYY)', value: '2.364', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      tripleX: { label: 'HC Trisomy X (47, XXX)', value: '2.364', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' }
    };
  ...
  async generatePdf(sampleData) {
    ...
    if (dateParts.length === 3) {
      ...
      firstPage.drawText(dateParts[2], { x: 518, y: 222.3, size: 9, font: fontBold, color: darkColor });
    }

    // ================= PAGE 2 OVERLAY =================
    if (secondPage) {
      drawRightText(secondPage, sampleData.fullName || '', RIGHT_COL1_X, 761.5, fontBold, 10.5, textColor);
      drawRightText(secondPage, sampleData.sampleCode || '', RIGHT_COL3_X, 762.5, fontBold, 10.5, textColor);
