  ...
  async parsePdf(pdfBuffer) {
    ...
    const extractTrisomy = (pattern) => {
      ...
      while ((m = regex.exec(text)) !== null) {
        ...
        const snippet = text.slice(startIdx, startIdx + 250);
        ...
        const decMatches = snippet.match(/(-?[0-9]{1,2}[.,][0-9]{1,4})/g);
        if (decMatches && decMatches.length > 0) {
          for (const dec of decMatches) {
            const val = dec.replace(',', '.');
        ...
        const zMatch = snippet.match(/(?:Z-score|Zscore|Z)\s*[:=]?\s*(-?[0-9]+(?:[.,][0-9]+)?)/i);
        if (zMatch) {
          const val = zMatch[1].replace(',', '.');
          return { value: val, risk };
        }
      }
