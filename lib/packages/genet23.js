  ...
  async parsePdf(pdfBuffer) {
    ...
    const extractTrisomy = (pattern) => {
      ...
      while ((m = regex.exec(text)) !== null) {
        ...
        const snippet = text.slice(startIdx, startIdx + 1200);

        let risk = 'Nguy cơ thấp';
        if (/nguy\s*cơ\s*cao/i.test(snippet)) {
          risk = 'Nguy cơ cao';
        } else if (/nguy\s*cơ\s*thấp/i.test(snippet)) {
          risk = 'Nguy cơ thấp';
        }

        const decMatches = snippet.match(/([-–—−]?\s*[0-9]{1,2}[.,][0-9]{1,4})/g);
        if (decMatches && decMatches.length > 0) {
          for (const dec of decMatches) {
            const val = dec.replace(/\s+/g, '').replace(/[-–—−]/g, '-').replace(',', '.');
            const num = parseFloat(val);
            if (!isNaN(num) && Math.abs(num) <= 50) {
              return { value: val, risk };
            }
          }
        }

        const zMatch = snippet.match(/(?:Z-score|Zscore|Z)\s*[:=]?\s*([-–—−]?\s*[0-9]+(?:[.,][0-9]+)?)/i);
        if (zMatch) {
          const val = zMatch[1].replace(/\s+/g, '').replace(/[-–—−]/g, '-').replace(',', '.');
          return { value: val, risk };
        }
      }
    ...
    otherTrisomies.forEach(num => {
      const p = (num === 1)
        ? `Trisomy\\s*[-_\\s]*(?:1|l|I|\\||01)(?:\\b|\\s|$)`
        : `Trisomy\\s*[-_\\s]*0?${num}\\b`;
      const match = extractTrisomy(p);
