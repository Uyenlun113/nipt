...
export const package20gaHandler = {
  ...
  async generatePdf(sampleData) {
    ...
    // Nơi gửi mẫu: (Right-aligned to Column 1 right edge RIGHT_COL1_X = 234) & Ngày trả kết quả (Column 3)
    drawRightText(sampleData.facilityName || '', RIGHT_COL1_X, 635.0, font, 8.5, darkColor);
    drawRightText(formattedReportDate || '', RIGHT_COL3_X, 635.0, font, 9.5, darkColor);
    ...
    if (pages.length > 1) {
      ...
      if (checkerToUse) {
        drawText(page2, checkerToUse, 100, 512.0, 9.5, fontBold, darkColor);
      ...
      if (directorToUse) {
        drawText(page2, directorToUse, 400, 512.0, 9.5, fontBold, darkColor);
