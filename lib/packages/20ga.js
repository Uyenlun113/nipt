...
import { sync20GAResultsWithConclusion } from '../20ga-utils.js';

export { sync20GAResultsWithConclusion };
...
export const package20gaHandler = {
  ...
  async parsePdf(pdfBuffer) {
    ...
    let gender = 'Nữ';
    const genderMatch = text.match(/Giới\s*tính:\s*(Nam|Nữ)/i);
    if (genderMatch) gender = genderMatch[1].toLowerCase() === 'nam' ? 'Nam' : 'Nữ';
    ...
    let reportDate = '';
    const rDateMatch = text.match(/Ngày\s*trả\s*kết\s*quả:\s*([^\n\r]+)/i) || text.match(/Ngày\s*trả\s*KQ:\s*([^\n\r]+)/i);
    if (rDateMatch) reportDate = rDateMatch[1].trim();

    let conclusion = '';
    const klMatch = text.match(/KẾT\s*LUẬN\s*:?[\s\S]*?(?=LƯU\s*Ý|Thông\s*tin\s*xét\s*nghiệm|Ngày|KIỂM\s*SOÁT|Hà\s*Nội|$)/i);
    if (klMatch) {
    ...
    conclusion = conclusion.replace(/\s*,?\s*Hà\s*Nội[\s\S]*$/i, '').trim();
    conclusion = conclusion.replace(/^Hà\s*Nội,?\s*/i, '').trim();
    if (!conclusion) {
    ...
    results = sync20GAResultsWithConclusion(results, conclusion);
  ...
  async generatePdf(sampleData) {
    const pdfBuffer = getCachedPdfTemplate(this.templateFileName, '20GA.pdf');
    ...
    const defaultResults = this.getDefaultResults();
    const rawResults = (sampleData.results20GA && Object.keys(sampleData.results20GA).length > 0)
      ? sampleData.results20GA
      : (sampleData.results || defaultResults);

    const pkgType = (sampleData.packageType || '').toUpperCase();
    const isSingle20GA = !pkgType || pkgType === '20GA' || (pkgType.includes('20GA') && !pkgType.includes('+'));

    let conclusionStr = isSingle20GA
    ...
    conclusionStr = (conclusionStr || '').replace(/^(KẾT\s*LUẬN|Kết\s*quả)\s*:?\s*/i, '').trim();

    const res = sync20GAResultsWithConclusion(rawResults, conclusionStr);
    const yMap = [
      544.0, 526.0, 510.0, 494.0, 478.0, 462.0, 442.0, 420.0, 404.0, 388.0,
      368.0, 347.0, 323.0, 303.0, 282.0, 266.0, 244.0, 224.0, 204.0, 179.0
    ];

    let hasAnyMutation = false;
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      if (typeof item === 'object' && item !== null) {
        val = item.value !== undefined ? item.value : defaultItem.value;
      } else if (typeof item === 'string') {
        val = item;
      }

      const valLower = String(val).toLowerCase();
      const isMutated = val && !valLower.includes('chưa phát hiện đột biến') && !valLower.includes('chưa phát hiện biến thể');
      if (isMutated) {
        hasAnyMutation = true;
      }
      ...
      if (y) {
        page1.drawText(String(val), {
          ...
          y: y,
          size: 7.5,
          font: isMutated ? fontBold : font,
          color: isMutated ? alertColor : darkColor,
          maxWidth: 175,
          lineHeight: 9
        });
      }
    ...
    // KẾT LUẬN (y=139.0, template has static "KẾT LUẬN:" at x=35.81)
    if (!conclusionStr) {
      conclusionStr = 'Chưa phát hiện biến thể gây bệnh/ có thể gây bệnh trên các vùng gen được khảo sát.';
    }
    const concLower = conclusionStr.toLowerCase();
    const isConcMutated = hasAnyMutation || (!concLower.includes('chưa phát hiện') && !concLower.includes('chua phat hien'));
    ...
    page1.drawText(conclusionStr, {
      ...
      size: 9,
      font: fontBold,
      color: isConcMutated ? alertColor : textColor,
      maxWidth: 470,
      lineHeight: 12
    });
    ...
    if (pages.length > 1) {
      const dateToUse = formatDateVN(sampleData.reportDate20GA) || formattedReportDate;
      const checkerToUse = sampleData.checkerName20GA !== undefined && sampleData.checkerName20GA !== '' ? sampleData.checkerName20GA : sampleData.checkerName;
      const directorToUse = sampleData.directorName20GA !== undefined && sampleData.directorName20GA !== '' ? sampleData.directorName20GA : sampleData.directorName;
      const hasMstStampToUse = sampleData.hasMstStamp20GA !== undefined && sampleData.hasMstStamp20GA !== false ? sampleData.hasMstStamp20GA : sampleData.hasMstStamp;

      const dateParts = dateToUse.split('/');
      if (dateParts.length === 3) {
        drawText(page2, dateParts[0], 406, 606.0, 9.5, fontBold, darkColor);
        drawText(page2, dateParts[1], 454, 606.0, 9.5, fontBold, darkColor);
        drawText(page2, dateParts[2].slice(-2), 506, 606.0, 9.5, fontBold, darkColor);
      }

      const CHECKER_CENTER_X = 161.0;
      const DIRECTOR_CENTER_X = 441.0;

      const drawCenterTextPage2 = (text, centerX, y, selectedFont, size, color) => {
        if (!text) return;
        const str = String(text);
        const textWidth = selectedFont.widthOfTextAtSize(str, size);
        page2.drawText(str, { x: centerX - (textWidth / 2), y, size, font: selectedFont, color });
      };

      if (checkerToUse) {
        drawCenterTextPage2(checkerToUse, CHECKER_CENTER_X, 512.0, fontBold, 9.5, darkColor);
      ...
      if (directorToUse) {
        drawCenterTextPage2(directorToUse, DIRECTOR_CENTER_X, 512.0, fontBold, 9.5, darkColor);
