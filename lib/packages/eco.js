...
import { getCachedFontBytes, getCachedPdfTemplate, getCachedMstStampBytes } from '../file-cache.js';
...
export const ecoPackageHandler = {
  ...
  async generatePdf(sampleData) {
    ...
    if (res.t21) {
      drawCenterText(res.t21.value || '-0.250', TABLE_VAL_CENTER_X, 489.3, fontBold, 9.5, darkColor);
    ...
    if (res.t18) {
      drawCenterText(res.t18.value || '0.351', TABLE_VAL_CENTER_X, 470.8, fontBold, 9.5, darkColor);
    ...
    if (res.t13) {
      drawCenterText(res.t13.value || '-0.469', TABLE_VAL_CENTER_X, 452.3, fontBold, 9.5, darkColor);
