...
export const ecoPackageHandler = {
  id: 'GeneT Eco',
  name: 'GeneT Eco',
  templateFileName: 'KQ_NIPT_GENET Eco.pdf',

  // 1. Get default result fields for Eco (ONLY T21, T18, T13 - Empty values)
  getDefaultResults() {
    return {
  ...
  // 2. Parse result PDF for Eco (Basic.pdf)
  async parsePdf(pdfBuffer) {
    ...
    // Extract "Phiên giải kết quả:" text
    let conclusion = '';
  ...
  // 3. Generate populated GeneTrust Eco PDF template (Page 1 ONLY)
  async generatePdf(sampleData) {
    const pdfBuffer = getCachedPdfTemplate(this.templateFileName, 'KQ_NIPT_GENET Eco.pdf');
    ...
    const firstPage = pages[0]; // PAGE 1 ONLY FOR ECO
    ...
    const renderRow1 = (itemKey, y) => {
      ...
      drawCenterText(item.risk || 'Nguy cơ thấp', TABLE_RISK_CENTER_X, y, fontBold, 9.5, isAlert ? alertColor : textColor);
    };
    ...
    // Date line (Hà Nội, ngày... tháng... năm... tại y = 238.7)
    ...
    if (dateParts.length === 3) {
      firstPage.drawText(dateParts[0], { x: 406, y: 238.7, size: 9, font: fontBold, color: darkColor });
      firstPage.drawText(dateParts[1], { x: 466, y: 238.7, size: 9, font: fontBold, color: darkColor });
      firstPage.drawText(dateParts[2], { x: 518, y: 238.7, size: 9, font: fontBold, color: darkColor });
    }
    ...
    if (sampleData.checkerName) {
      drawCenterText(sampleData.checkerName, CHECKER_CENTER_X, 150.0, fontBold, 9.5, darkColor);
    ...
    if (sampleData.directorName) {
      drawCenterText(sampleData.directorName, DIRECTOR_CENTER_X, 150.0, fontBold, 9.5, darkColor);
