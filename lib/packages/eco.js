...
export const ecoPackageHandler = {
  ...
  async generatePdf(sampleData) {
    ...
    const templatePath = path.join(phoiDir, this.templateFileName);
    ...
    // Address at x = 68 to prevent overlap
    ...
    // Date line (Hà Nội, ngày... tháng... năm... on Page 1 at y = 238.7)
    const dateToUse = formattedReceivedDate || formatDateVN(new Date().toISOString().split('T')[0]);
    const dateParts = dateToUse.split('/');
    if (dateParts.length === 3) {
      firstPage.drawText(dateParts[0], { x: 408, y: 238.7, size: 9, font: fontBold, color: darkColor });
