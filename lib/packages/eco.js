...
import { getCachedFontBytes, getCachedPdfTemplate, getCachedMstStampBytes, getCachedDirectorSignatureBytes } from '../file-cache.js';
...
export const ecoPackageHandler = {
  ...
  async parsePdf(pdfBuffer) {
    ...
    let conclusion = '';
    const pgMatch = text.match(/Phiên\s*giải\s*kết\s*quả\s*:?/i);
    if (pgMatch) {
      const pgIndex = pgMatch.index + pgMatch[0].length;
      let rawPg = text.slice(pgIndex, pgIndex + 500);
      const stopIndex = rawPg.search(/Lưu\s*ý|Khuyến\s*cáo|Ghi\s*chú|Chú\s*ý|Họ\s*tên|Mã\s*số\s*XN|Người\s*XN|Trang\s+[0-9]+|1[\)\.\-]|2[\)\.\-]/i);
      if (stopIndex !== -1) {
