...
export const ecoPackageHandler = {
  ...
  // 1. Get default result fields for Eco (ONLY T21, T18, T13)
  getDefaultResults() {
    return {
      t21: { label: 'Trisomy 21 (Down)', value: '-0.250', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      t18: { label: 'Trisomy 18 (Edwards)', value: '0.351', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' },
      t13: { label: 'Trisomy 13 (Patau)', value: '-0.469', risk: 'Nguy cơ thấp', ref: '-3 < Z < 3' }
    };
  },
  ...
  async generatePdf(sampleData) {
    ...
    if (dateParts.length === 3) {
      firstPage.drawText(dateParts[0], { x: 415, y: 383.4, size: 9, font: fontBold, color: darkColor });
      firstPage.drawText(dateParts[1], { x: 460, y: 383.4, size: 9, font: fontBold, color: darkColor });
      firstPage.drawText(dateParts[2], { x: 502, y: 383.4, size: 9, font: fontBold, color: darkColor });
    }
