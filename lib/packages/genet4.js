...
export const genet4PackageHandler = {
  ...
  async parsePdf(pdfBuffer) {
    ...
    // Extract Turner XO
    const turner = extractTrisomy('(?:Turner|45,\\s*XO|\\bXO\\b)');
    if (turner) {
  ...
  async generatePdf(sampleData) {
    ...
    // Medical Conclusion on Page 1 (Dịch lên y = 363.2 để khớp hoàn toàn nhãn "Kết luận:")
    ...
    // Date line on Page 1 (Hà Nội, ngày... tháng... năm... at y = 264.8)
    const dateToUse = formattedReceivedDate || formatDateVN(new Date().toISOString().split('T')[0]);
    const dateParts = dateToUse.split('/');
    if (dateParts.length === 3) {
      firstPage.drawText(dateParts[0], { x: 392, y: 264.8, size: 9, font: fontBold, color: darkColor });
      firstPage.drawText(dateParts[1], { x: 448, y: 264.8, size: 9, font: fontBold, color: darkColor });
      firstPage.drawText(dateParts[2], { x: 498, y: 264.8, size: 9, font: fontBold, color: darkColor });
    }
