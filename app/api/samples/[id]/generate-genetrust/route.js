...
import { generateGeneTrustPdf } from '@/lib/pdf-generator';
...
export async function GET(req, { params }) {
  try {
    ...
    // Generate populated GeneTrust PDF template
    const pdfBuffer = await generateGeneTrustPdf(sample);

    const safeName = (sample.fullName || 'KhachHang').replace(/[^a-zA-Z0-9]/g, '_');
    const fileName = `Ket_Qua_GeneTrust_${sample.sampleCode || 'NIPT'}_${safeName}.pdf`;
    ...
    console.error('GeneTrust PDF generation error:', error);
    return NextResponse.json({ error: 'Không thể tạo file phôi kết quả GeneTrust: ' + error.message }, { status: 500 });
  }
}
