...
export default function TwinsPackageListPage() {
  ...
  const fetchSamples = async () => {
    setLoading(true);
    try {
      let url = `/api/samples?package=${encodeURIComponent('GeneT Twins')}`;
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-6 w-full">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-200 w-full">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                    Quản Lý Gói NIPT GeneT Twins (Song Thai)
                  </h1>
                  <p className="text-sm text-slate-500 font-medium">
                    Gói xét nghiệm dành riêng cho mẹ mang Song thai • Phát hiện lệch bội 3 NST phổ biến: T21, T18, T13
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/samples/create"
                className="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-extrabold text-sm shadow-md shadow-rose-600/20 transition-all flex items-center gap-2"
              >
                <PlusCircle className="w-4 h-4" />
                <span>Tạo Mẫu Twins Mới</span>
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase">Tổng Mẫu NIPT GeneT Twins (Song Thai)</p>
                <h3 className="text-3xl font-black text-slate-900 mt-1">{samples.length}</h3>
              </div>
              <div className="w-12 h-12 bg-rose-50 border border-rose-100 rounded-2xl flex items-center justify-center text-rose-600 font-black">
                {samples.length}
              </div>
            </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:border-rose-500 transition-all"
              />
            </div>
          </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={8} className="py-12 text-center text-slate-400">
                        <RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2 text-[#274d96]" />
                        <span>Đang tải danh sách mẫu NIPT GeneT Twins</span>
                      </td>
                    </tr>
                  ) : samples.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="py-12 text-center text-slate-400">
                        Chưa có mẫu xét nghiệm NIPT GeneT Twins nào trong danh sách.
                      </td>
                    </tr>
                  ) : (
                    samples.map((s) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-4 px-4 font-mono font-extrabold text-[#274d96] text-sm">
