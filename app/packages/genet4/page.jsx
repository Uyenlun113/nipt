...
export default function GeneT4PackageListPage() {
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-6 w-full">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-200 w-full">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                    Quản Lý Gói NIPT GeneT 4 (Song Thai)
                  </h1>
                  <p className="text-sm text-slate-500 font-medium">
                    Sàng lọc 4 hội chứng di truyền: Down (T21), Edwards (T18), Patau (T13) & Hội chứng Turner (XO)
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/samples/create"
                className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-extrabold text-sm shadow-md shadow-amber-600/20 transition-all flex items-center gap-2"
              >
                <PlusCircle className="w-4 h-4" />
                <span>Tạo Mẫu GeneT 4 Mới</span>
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase">Tổng Mẫu NIPT GeneT 4 (Song Thai)</p>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:border-amber-500 transition-all"
              />
            </div>
          </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-slate-400">
                        ...
                        <span>Đang tải danh sách mẫu NIPT GeneT 4 (Song Thai)...</span>
                      </td>
                    </tr>
                  ) : samples.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-slate-400">
                        Chưa có mẫu xét nghiệm NIPT GeneT 4 (Song Thai) nào trong danh sách.
                      </td>
                    </tr>
                  ) : (
                    samples.map((s) => {
                      ...
                      const detailUrl = '/samples/genet4/' + id;

                      return (
                        <tr key={id} className="hover:bg-slate-50/70 transition-colors">
                          <td className="py-4 px-4 font-mono font-extrabold text-amber-900 text-sm">
                          ...
                          <td className="py-4 px-4 text-center">
                            {hasCfDna ? (
                              <span className="px-3 py-1 rounded-lg bg-amber-50 text-amber-900 font-mono font-extrabold text-xs border border-amber-200 shadow-xs">
                                {s.cfDNA}%
                              </span>
                            ) : (
                          ...
                          <td className="py-4 px-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                ...
                                className="px-3.5 py-1.5 bg-gradient-to-r from-amber-600 to-indigo-600 hover:from-amber-700 hover:to-indigo-700 text-white rounded-xl text-xs font-extrabold transition-all shadow-xs flex items-center gap-1.5"
                                title="Tải về cả 2 file PDF (NIPT GeneT 4 + Kết quả phụ GBS)"
                              >
