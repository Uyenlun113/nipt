...
export default function GeneT23PackageListPage() {
  ...
  const fetchSamples = async () => {
    setLoading(true);
    try {
      let url = `/api/samples?package=${encodeURIComponent('GeneT 23')}`;
      if (searchTerm) {
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-6 w-full">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-200 w-full">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                    Quản Lý Gói NIPT GeneT 23 (Song Thai)
                  </h1>
                  <p className="text-sm text-slate-500 font-medium">
                    Khảo sát toàn diện số lượng trên toàn bộ 23 cặp nhiễm sắc thể người (22 NST thường + 1 cặp NST giới tính)
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/samples/create"
                className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-extrabold text-sm shadow-md shadow-indigo-600/20 transition-all flex items-center gap-2"
              >
                <PlusCircle className="w-4 h-4" />
                <span>Tạo Mẫu GeneT 23 Mới</span>
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase">Tổng Mẫu NIPT GeneT 23 (Song Thai)</p>
                <h3 className="text-3xl font-black text-slate-900 mt-1">{samples.length}</h3>
              </div>
              <div className="w-12 h-12 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 font-black">
                {samples.length}
              </div>
            </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:border-indigo-500 transition-all"
              />
            </div>
          </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-slate-400">
                        ...
                        <span>Đang tải danh sách mẫu NIPT GeneT 23 (Song Thai)...</span>
                      </td>
                    </tr>
                  ) : samples.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-slate-400">
                        Chưa có mẫu xét nghiệm NIPT GeneT 23 (Song Thai) nào trong danh sách.
                      </td>
                    </tr>
                  ) : (
                    samples.map((s) => {
                      ...
                      const detailUrl = '/samples/genet23/' + id;
                      ...
                      return (
                        <tr key={id} className="hover:bg-slate-50/70 transition-colors">
                          ...
                          <td className="py-4 px-4 text-slate-700 font-medium">
                            ...
                            <span className="inline-block mt-0.5 px-2 py-0.5 rounded text-[11px] font-bold bg-indigo-50 text-indigo-800 border border-indigo-200">
                          ...
                          <td className="py-4 px-4 text-center">
                            {hasCfDna ? (
                              <span className="px-3 py-1 rounded-lg bg-indigo-50 text-indigo-900 font-mono font-extrabold text-xs border border-indigo-200 shadow-xs">
                                {s.cfDNA}%
                              </span>
                            ) : (
